...
public class BindingTest {
	...
	public void shouldFindPostsInList() {
		try (SqlSession session = sqlSessionFactory.openSession()) {
			...
			List<Post> posts = mapper.findPostsInList(new ArrayList<Integer>() {
				private static final long serialVersionUID = -7793079960757213420L;
	...
	public void shouldInsertAuthorWithSelectKey() {
		try (SqlSession session = sqlSessionFactory.openSession()) {
			BoundAuthorMapper mapper = session.getMapper(BoundAuthorMapper.class);
			Author author = new Author(-1, "cbegin", "******", "cbegin@nowhere.com", "N/A",
				Section.NEWS);
	...
	public void shouldSelectListOfPostsLikeTwoParameters() {
		try (SqlSession session = sqlSessionFactory.openSession()) {
			...
			List<Post> posts = mapper
				.selectPostsLikeSubjectAndBody(new RowBounds(1, 1), "%a%", "%a%");
			assertEquals(1, posts.size());
		}
	}
	...
	public void shouldSelectOneBlogAsMap() {
		try (SqlSession session = sqlSessionFactory.openSession()) {
			...
			Map<String, Object> blog = mapper.selectBlogAsMap(new HashMap<String, Object>() {
				private static final long serialVersionUID = 8644853005685595761L;
	...
	public void shouldCacheMapperMethod() throws Exception {
		try (SqlSession session = sqlSessionFactory.openSession()) {
			...
			final MapperMethod cachedSelectBlog = mapperProxyFactory.getMethodCache()
				.get(selectBlog);
	...
	public void registeredMappers() {
		Collection<Class<?>> mapperClasses = sqlSessionFactory.getConfiguration()
